interface DraftMetadataProps {
  players: { current: number; max: number; isMax?: boolean }
  timer: number
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-8 gap-y-2 text-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-foreground">{players.current}</span>
      </div>
      <div className="flex items-center gap-2">
        <span className="font-bold text-foreground">TIMER</span>
        <span className="font-mono text-foreground">
          {timer === 0 ? 'UNTIMED' : `${timer}s`}
        </span>
      </div>
      <div className="flex items-center gap-2">
